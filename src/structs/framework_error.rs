...
impl<'a, U: Debug, E: Debug> Debug for FrameworkError<'_, U, E> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::EventHandler { error, ctx, event, framework } => f.debug_struct("EventHandler").field("error", error).field("ctx", ctx).field("event", event).finish(),
            ...
            Self::UnknownCommand { ctx, msg, prefix, msg_content, framework, invocation_data, trigger } => f.debug_struct("UnknownCommand").field("msg", msg).field("prefix", prefix).field("msg_content", msg_content).field("trigger", trigger).finish(),
            Self::UnknownInteraction { ctx, framework, interaction } => f.debug_struct("UnknownInteraction").field("interaction", interaction).finish(),
            Self::NonCommandMessage { error, ctx, framework, msg } => f.debug_struct("NonCommandMessage").field("error", error).field("msg", msg).finish(),
